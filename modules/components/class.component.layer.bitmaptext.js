...
/**
 * Класс слоя для компонента. В качестве слоя использует bitmaptext.
 */
class ComponentLayerBitmapText {
    ...
    constructor(component, config) {
        ...
        console.log('ComponentLayerBitmapText.constructor');
        ...
        // запоминаем size.
        // this.size = (typeof config.size === 'number' ? config.size : 32);
 
        // запоминаем align.
        // this.align = (config.align && typeof config.align === 'string' ? config.align : 'center');
        
        // запоминаем шрифт.
        // this.fontName = (config.fontName && typeof config.fontName ==='string')?config.fontName:'Arial';
        ...
        // this.sprite = this.scene.add.bitmapText(0, 0, this.fontName, config.text, this.size);
     ...
     * 
     * @returns {ComponentLayerBitmapText} ссылка на самого себя.
     */
    destroy() {
     ...
     * анимации по времени или методами {@link ComponentLayerBitmapText#stop ComponentLayerBitmapText.stop()} и {@link ComponentView#stop ComponentView.stop()}.
     * 
     * @returns {ComponentLayerBitmapText} ссылка на самого себя.
     */
     ...
     * !!! не может применятся всесте с методами {@link ComponentLayerBitmapText#setHue ComponentLayerBitmapText.setHue()} и {@link ComponentLayerBitmapText#setIllumination ComponentLayerBitmapText.setIllumination()},
     ...
     * !!! не может применятся всесте с методами {@link ComponentLayerBitmapText#setTint ComponentLayerBitmapText.setTint()} и {@link ComponentLayerBitmapText#setIllumination ComponentLayerBitmapText.setIllumination()},
     * так как все три метода работают путем устанавки свойства tint.
    ...
    /**
     * Меняет освещенность слоя в зависимости от illumination.
     * !!! не может применятся всесте с методами {@link ComponentLayerBitmapText#setTint ComponentLayerBitmapText.setTint()} и {@link ComponentLayerBitmapText#setHue ComponentLayerBitmapText.setHue()},
     * так как все три метода работают путем устанавки свойства tint.
     * @param {number} illumination число от 0 (черный) до 1 (оригинальный).
     * 
     * @returns {ComponentLayerBitmapText} ссылка на самого себя.
     ...
     */
    setText(text) {
        if (['number', 'string'].indexOf(typeof text) < 0) text = '';
...
export default ComponentLayerBitmapText;
